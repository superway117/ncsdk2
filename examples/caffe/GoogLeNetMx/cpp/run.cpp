...
const int channelDim = 3;
const int paddedChannelDim = 3;
...
float *LoadImage(const char *path, int reqsize, float *mean, unsigned int* imageSize)
{
	...
	img = stbi_load(path, &width, &height, &cp, channelDim);
	if(!img)
	{
		printf("The picture %s could not be loaded\n", path);
		return 0;
	}
	imgresized = (unsigned char*) malloc(channelDim*reqsize*reqsize);
	if(!imgresized)
	{
		free(img);
		perror("malloc");
	...
	stbir_resize_uint8(img, width, height, 0, imgresized, reqsize, reqsize, 0, channelDim);
	free(img);
    *imageSize = sizeof(*imgfp32) * reqsize * reqsize * paddedChannelDim;
	...
	for (int c = 0; c < channelDim; c++)
	{
		unsigned int srcChannelOffset = 2 - c;
		unsigned int meanIndex = c;
		unsigned int dstChannelOffset = c * reqsize*reqsize;
		for (int y = 0; y < reqsize*reqsize; y++)
		{
			unsigned char color = imgresized[srcChannelOffset + y*channelDim];
			float value = static_cast<float>(color) - mean[meanIndex];
			imgfp32[dstChannelOffset + y] = value;
		}
	}
	return imgfp32;
}
...
int main(int argc, char** argv)
{
    ...
    {   // successfully allocated graph.  Now graphHandle is ready to go.  
        ...
        hostTensorDesc.wStride = sizeof(float); //FP32
        hostTensorDesc.hStride = hostTensorDesc.wStride * hostTensorDesc.w;
        hostTensorDesc.cStride = hostTensorDesc.hStride * hostTensorDesc.h;
        hostTensorDesc.totalSize = hostTensorDesc.cStride * hostTensorDesc.c;
        length = sizeof(struct ncTensorDescriptor_t);
        retCode = ncFifoSetOption(bufferIn, NC_RW_FIFO_HOST_TENSOR_DESCRIPTOR, &hostTensorDesc, length);
        if (retCode != NC_OK)
        {
            printf("Error - Input Fifo set host tensor descriptor failed!");
            exit(-1);
        }

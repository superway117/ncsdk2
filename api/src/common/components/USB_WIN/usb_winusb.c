...
//	Port_#0001.Hub_#0004
	...
	//sprintf(buff, "%u/%u", hub, port);
	sprintf(buff, "%u.%u-%s", port, hub, usb_get_pid_name(pid));
	return buff;
ret_err:
	return "<error>";
}
...
int usb_list_devices(uint16_t vid, uint16_t pid, uint8_t dev_des[][2 + 2 + 4 * 7 + 7]) {
	...
	for(i=0; SetupDiEnumDeviceInfo(devInfo, i, &devInfoData); i++) {
		if(!SetupDiGetDeviceRegistryProperty(devInfo, &devInfoData, SPDRP_HARDWAREID, NULL, hwid_buff, sizeof(hwid_buff), NULL))
			continue;
		uint16_t fvid, fpid;
		if(sscanf(hwid_buff, "USB\\VID_%hx&PID_%hx", &fvid, &fpid) != 2)
			continue;
		...
		//char *add = str;
		//add = (*fn)(fpid);
		//add = usb_get_pid_name(fpid);
		//char *p = (char *)&dev_des[i][4];
		//(fpid == DEFAULT_OPENPID) ? gen_addr(devInfo, &devInfoData, get_pid_by_name(addr)) : gen_addr(devInfo, &devInfoData, fpid)
		sprintf((char *)&dev_des[i][4], "%s", gen_addr(devInfo, &devInfoData, fpid));
		//fprintf(stderr, "dev_des[i][4]:=%s\n", (char *)&dev_des[i][4]);
		if((fvid == vid) && (fpid == pid)) {
			//printf("Address: %-7s - VID/PID %04x:%04x\n", gen_addr(devInfo, &devInfoData, fpid), fvid, fpid);
		}
	}
	SetupDiDestroyDeviceInfoList(devInfo);
	return i;

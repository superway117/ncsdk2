...
///
/// @brief    USB VSC oucall functions
///
    ...
    case USBPUMP_PROTO_VSC2_EVENT_INTERFACE_UP:
      {
        ...
        for (ep = 1; ep <= 3; ep++) {
          sem_post(&pSelf->semWriteId[ep - 1]);
          sem_post(&pSelf->semReadId[ep - 1]);
        }
      }
      break;
    ...
    )
{
  ...
  {
    sem_post(&pSelf->semReadId[endPoint-1]);
  }
    ...
    )
{
  ...
  {
    sem_post(&pSelf->semWriteId[endPoint-1]);
  }
